...
const USER_NARRATIVE_PATTERN = /\b(?:i\s+(?:(?:ve|m)\s+)?(?:use|used|using|am\s+using|prefer|recommend|hate|love)|we\s+(?:ve\s+)?(?:use|used|using|are\s+using|prefer|recommend)|my\s+experience|our\s+experience|kullaniyorum|kullandim|kullandik|kullaniyoruz|deneyimim|deneyimimiz|tecrubem|tecrubemiz|oner(?:irim|iyorum)|memnunum|nefret\s+ettim)\b/i;
